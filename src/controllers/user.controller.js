...
export const getUserPlaylist = async (req, res) => {
  ...
  try {
    ...
    const result = await axios.get("https://api.spotify.com/v1/me/playlists?limit=50", 
      {
        headers : { Authorization : `Bearer ${accessToken}`}
      }
    )
...
export const getUserFollowing = async (req, res) => {
  ...
  try {
    ...
    const result = await axios.get("https://api.spotify.com/v1/me/following?type=artist&limit=50", 
    {
      headers : { Authorization : `Bearer ${accessToken}`}
    }
  )
